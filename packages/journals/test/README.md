...
| Fixed-duration waits in async suites                        | Many suites settle async pipelines with `await wait(n)`; single-stage rAF flushes are deterministic in the setTimeout-backed harness, but multi-stage pipelines can race on loaded CI runners — `Register.workerPool.test.ts` and `EntryStream.announcements.test.ts` flaked exactly this way and now settle by quiescence instead. | Medium   | Convert waits to quiescence polling when a suite flakes; deadline-bounded  |
