...
declare global {
  interface Window {
    __registerStyleIsolationReady?: boolean;
  }
}

async function openFixture(page: Page) {
  await page.goto('/test/e2e/fixtures/register-style-isolation.html');
  await page.waitForFunction(
    () => window.__registerStyleIsolationReady === true
  );
}

test.describe('register shadow style isolation', () => {
  test('hostile page CSS does not leak into shadow register rows', async ({
    page,
  }) => {
    await openFixture(page);

    const rowStyles = await page.evaluate(() => {
      const host = document.getElementById('test-register-host');
  ...
  test('register internal styles do not leak out to light DOM pseudo rows', async ({
    page,
  }) => {
    await openFixture(page);
    ...
    // sees the page's own hostile rules (red background, inline display) —
    // never the component's selected-row background or grid layout.
  ...
  test('css custom properties flow into the shadow root for theming', async ({
    ...
    const headerBackground = await page.evaluate(() => {
      const host = document.getElementById('test-register-host');
      const header = host?.shadowRoot?.querySelector('[data-register-header]');
      return header != null ? getComputedStyle(header).backgroundColor : null;
    });

    expect(headerBackground).toBe('rgb(1, 2, 3)');
  });
});
