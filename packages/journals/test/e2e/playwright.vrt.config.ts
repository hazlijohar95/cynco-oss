...
// root); the journals:test-vrt moon task injects it via envFile.
loadWorktreeEnv();

const rawOffset = Number(process.env.CYNCO_PORT_OFFSET ?? 0);
const portOffset = Number.isFinite(rawOffset) ? rawOffset : 0;
// The VRT lane binds its own port two above the registered journalsE2e base
// (4283, see PORT_BASES in scripts/wt.ts) so `moon run journals:test-e2e
// journals:test-vrt` can share a pipeline without the two webServers
...
// together, and +2 stays clear of accountsE2e at base+100.
const vrtPort = 4285 + portOffset;
...
const config: PlaywrightTestConfig = defineConfig({
  ...
  outputDir: `/tmp/cynco-journals-vrt-results${portOffset > 0 ? `-${portOffset}` : ''}`,
  ...
  // journals:test-vrt -- --update-snapshots` never reaches playwright), so
  // intentional baseline refreshes opt in via the environment instead:
  // `VRT_UPDATE_SNAPSHOTS=1 moonx journals:test-vrt`. `changed` rewrites
  ...
  webServer: {
    command: `JOURNALS_E2E_PORT=${vrtPort} moon run journals:test-e2e-server`,
