...
// root); the accounts:test-e2e moon task injects it via envFile.
loadWorktreeEnv();
...
const config: PlaywrightTestConfig = defineConfig({
  ...
  outputDir: `/tmp/cynco-accounts-playwright-results${portOffset > 0 ? `-${portOffset}` : ''}`,
  fullyParallel: true,
  reporter: 'list',
  timeout: 30_000,
