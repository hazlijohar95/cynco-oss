...
The shared shapes (`StatementLine`, `Posting`, `LedgerEntry`) come from
[`@cynco/ledger-core`](/docs/ledger-core) — the same definitions the journals
...
- **Core data** — `MinorUnits`, `EntryFlag`
  ...
  `StatementLine` — re-exported from the engine, the same definitions
  `@cynco/journals` consumes.
